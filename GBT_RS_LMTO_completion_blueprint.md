...
\[
S(\mathbf d)\rightarrow S(\mathbf d)D(\alpha_{ij})
...
|---|---|---|---|---|
| `periodic_nc` | chemical-cell/per-type reference moments | none | per-type `ee`, per-site `hall` | ordinary collinear/noncollinear states |
| `gbt_single_q` | rotating-frame reference moments | \(D(\alpha)\) | per-type `ee`; optional per-site oracle | bulk single-\(q\) spirals/cones |
...
The existing ordinary Pauli algebra is shared by all three modes. Only the source of the endpoint moments and the presence of the one-sided GBT phase differ.
...
Store any additional, non-geometric sublattice phase in the rotating-frame reference moment \(\mathbf m_a^0\). Then

\[
\mathbf m_i=\mathbf m_a^0,
\qquad
\mathbf m_j=R_z(\alpha_{ij})\mathbf m_b^0,
\]

and **do not** add \(\chi_b-\chi_a\) to `alpha`. An alternative cell-translation convention is valid, but it must not coexist in production code.
...
- per-sublattice input supplies the reference orientation;
- no downstream routine invents another phase.

### 2.3 The pair shortcut is not a blanket transform

Right-multiplying the completed elementary pair by \(D(\alpha)\) is valid because

\[
W_j^{\rm lab}=D(\alpha)W_j^0D^\dagger(\alpha)
\]

and therefore

\[
W_iS W_j^{\rm lab}D(\alpha)=W_i[S D(\alpha)]W_j^0.
\]

It must not be applied blindly to every later object. The following need separate classification:

- on-site `enim`, `obarm`, constraining fields, Hubbard-U and other local terms: no translational phase, but use the same rotating-frame axis;
...
### 3.3 Pure noncollinear pair kernel

Refactor the useful part of `ham0m_nc` into a routine with explicit inputs:

```fortran
assemble_nc_pair(type_i, type_j, moment_i, moment_j, structure, pair_pauli)
```

It performs the current `wx0/wx1`, dot-product, cross-product, on-site, and Pauli algebra without knowing about `q_ss`, absolute positions, solver choice, or GBT.

### 3.4 GBT wrapper

The GBT wrapper performs exactly:

1. resolve \(\mathbf m_a^0\) and \(\mathbf m_b^0\);
2. compute \(\alpha\) once from the directed physical bond;
3. call the pure pair kernel with \(\mathbf m_a^0\) and \(R_z(\alpha)\mathbf m_b^0\);
4. right-multiply the resulting spin block by \(D(\alpha)\).

The half-angle Pauli update must use temporaries:

\[
\begin{aligned}
h_0'&=ch_0-ish_z,& h_z'&=ch_z-ish_0,\\
h_x'&=ch_x+sh_y,& h_y'&=ch_y-sh_x,
\end{aligned}
\]

where \(c=\cos(\alpha/2)\) and \(s=\sin(\alpha/2)\).

### 3.5 Explicit-texture wrapper

The explicit-texture wrapper obtains `moment_i` and `moment_j` from a site-indexed texture provider and calls the same pure pair kernel. It applies no \(D\) phase.
...
```text
GBT/NC bond builder -> ee/eeo/... -> recursion
...
**Gate G2:** on an ordinary magnetic k-space case, direct `EBAND` is stable against DOS grid/window changes, agrees with a converged total-DOS integral within its integration error, and is invariant under a \(q=0\) global spin rotation. The projected-DOS route may not be used for MFT even if it happens to agree.
...
2. Extract the pure `assemble_nc_pair` kernel.
...
4. Route `build_bulkham` and `build_locham` explicitly:
   ...
   - `gbt_single_q`: bond-gauge wrapper;
   - `explicit_texture`: per-site build only.
5. Add an explicit failure if arbitrary site moments are requested through per-type bulk reuse.
6. Retain `rotate_to_local_axis`/`rotate_from_local_axis` and existing general noncollinear tests.
...
### WP4 — Implement the single GBT bond kernel

1. Add the central phase helper and remove direct phase calculations from `ham0m_nc`.
2. Use independent `mom_i`, `mom_j0`, and `mom_j` temporaries.
3. Apply the relative endpoint rotation and trailing half-angle phase.
4. Use the same reference-axis resolver for on-site pair terms, `enim`, and `obarm`.
5. Add bond dumps keyed by directed geometry and compare them with the dense oracle.
6. Bypass all new floating-point work when GBT and cone inputs are inactive.

**Gate G4:** dense oracle, Fourier shifted-\(k\) identity, directed-bond Hermiticity, \(q\leftrightarrow-\mathbf q\), \(\theta=0\), and \(q=0\) global-rotation tests all pass.

### WP5 — Delete the reciprocal reconstruction and unify solvers

1. Remove `fourier_transform_gbt_array` and `fourier_transform_gbt` implementations, interfaces, and call sites.
...
|---|---|
| elementary `ee` | completed in WP4 |
| on-site `cx/cex`, `enim`, `obarm` | same rotating-frame reference axis; \(\alpha=0\) |
...
| CCOR | remove full-angle transverse rotations; gauge each earliest directed structure-like factor |
...
5. delete mutually exclusive cone/sublattice branching in favor of composed reference moments;
...
**Gate G10:** repository search finds one phase helper, one GBT bond wrapper, no reciprocal GBT reconstruction, no CCOR full-angle spiral rotation, and a fully tested explicit-texture path.
...
| pure Pauli pair algebra | `hamiltonian_build.f90::assemble_nc_pair` |
| GBT wrapper | `hamiltonian_build.f90` |
...
- One production GBT bond transformation exists.

  ...
  function constructor(label, database, reload) result(obj)
    ...
    if (reload_) then
      ...
      if(exists(path_to_file)) then
        if(rank==0)call g_logger%info('Reading potential from previous result '// trim(path_to_file), __FILE__, __LINE__)
    ...
    if (.not. built) then
      ...
      if(exists(path_to_file)) then
        ...
        if(exists(path_to_file)) then
          ...
          if(rank==0)call g_logger%fatal('Element '//trim(label)//' not found in any database', __FILE__, __LINE__)
        endif
      endif
    endif

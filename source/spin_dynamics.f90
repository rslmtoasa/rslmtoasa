...
contains
   ...
   subroutine writelammpstraj(this, num_atoms, spins, lattice, timestep)
      implicit none
      class(spin_dynamics), intent(inout)      :: this
      integer, intent(in)  :: num_atoms
      real(rp), intent(in) :: timestep
      real(rp), intent(in) :: spins(3, num_atoms)
      real(rp), intent(in) :: lattice(3, num_atoms)
      ! Local variables
      integer              :: i
      integer, parameter   :: unit_num = 40

      ! Open the file for the first timestep, otherwise append
      if (timestep == this%dt) then

! frozen_magnon: sweeps &hamiltonian's q_ss over a list of points (units of
! pi/alat, same convention as &hamiltonian q_ss) and evaluates the energy at
! each. The first q-point is always the reference point (full SCF is run there
! unconditionally; every other point's E(q)/omega(q) is measured relative to it).
!
...
namelist /frozen_magnon/ mode, output_file, q_file, n_q_points, q_ss_list

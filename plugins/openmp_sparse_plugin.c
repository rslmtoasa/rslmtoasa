...
void *rslmto_backend_create(const char *precision_mode, int block_dim, int n_sites, const char *library_name) {
    ...
    backend->library_name = dup_string(library_name != NULL ? library_name : "openmp");
            ...
            for (j = 0; j < block_dim; ++j) {
                for (i = 0; i < block_dim; ++i) {
                    ...
                    for (l = 0; l < block_dim; ++l) {
                        size_t psi_idx = (size_t) l + (size_t) j * (size_t) block_dim + (size_t) col * (size_t) block_dim * (size_t) block_dim;
                        if (trans_mode == 'c' || trans_mode == 'C') {
                            size_t op_idx = (size_t) l + (size_t) i * (size_t) block_dim + (size_t) entry * (size_t) block_dim * (size_t) block_dim;
                            accum += conj(op->blocks[op_idx]) * psi_in[psi_idx];
                        } else {
                            size_t op_idx = (size_t) i + (size_t) l * (size_t) block_dim + (size_t) entry * (size_t) block_dim * (size_t) block_dim;
...
const char *rslmto_backend_name(void) {
    return "rslmto_openmp_sparse_plugin";
}

const char *rslmto_backend_capabilities(void) {
#ifdef _OPENMP
    return "execution=openmp_cpu;precision=complex_fp64;operators=block_sparse,velocity,hoh;recurrences=apply_only;acceleration=rows_parallel";
#else
    return "execution=serial_cpu;precision=complex_fp64;operators=block_sparse,velocity,hoh;recurrences=apply_only;acceleration=openmp_not_enabled";
#endif
}

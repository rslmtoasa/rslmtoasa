...
def main() -> None:
    ...
    assert values["live_production_six_branch_candidate_count"] == "348"
    assert values["second_order_four_branch_candidate_count"] == "232"
    assert values["second_order_six_branch_candidate_count"] == "348"
    assert values["live_production_retained_product_dimension"] == "348"
    assert values["rank_sensitivity_stable"] == "T"
    assert values["total_S4_prod_retained_rank_tau1_tau10_tau100"] == "348 348 348"
    ...
    assert values["total_S6_2nd_minus_S4_2nd_rank_tau1"] == "116"
    ...
    assert number(values, "S4_prod_vs_S4_2nd_principal_minimum") > 1.0 - 1.0e-8
    assert number(values, "S4_prod_vs_S4_2nd_prod_outside") > 1.0e-2
    assert number(values, "S4_prod_vs_S4_2nd_second_order_outside") < 1.0e-8
    assert number(values, "S4_prod_vs_S6_2nd_principal_minimum") > 1.0 - 1.0e-8
    assert number(values, "S4_prod_vs_S6_2nd_prod_outside") < 1.0e-10
    assert number(values, "S4_prod_vs_S6_2nd_six_outside") < 1.0e-10
    ...
    assert number(values, "S4_2nd_full_candidate_residual_global") > 1.0e-8
    assert number(values, "full_candidate_residual_global") < 1.0e-10
    ...
    assert max(vector(values, "R20 maximum RMS", 2)) < 1.0e-10
    assert max(vector(values, "R02 maximum RMS", 2)) < 1.0e-10
    assert max(vector(values, "S4_2nd_R20 maximum RMS", 2)) > 1.0e-8
    assert max(vector(values, "S4_2nd_R02 maximum RMS", 2)) > 1.0e-8
    assert number(values, "S4_2nd_R20 median") > 0.0
    assert number(values, "S4_2nd_R02 median") > 0.0
    assert number(values, "mixed_LM_full_SR_residual") > 1.0e-8
    assert max(vector(values, "raw_pairing_abs", 4)) > 0.0
    assert max(vector(values, "projected_pairing_abs", 4)) > 0.0
    assert max(vector(values, "projection_loss", 4)) < 1.0e-10
    assert max(vector(values, "S4_2nd_projection_loss", 4)) > 1.0e-14
